...
class MessageController extends Controller
{
    ...
    public function getMsgAction()
    {
        ...
        return $this->render('MessageBundle:Message:get.html.twig', array(
            'messages' => $messages,
            'count' => $count,
        ));
    }
    ...
    //msgDiscussion.html
    public function msgDiscussionAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        if ($this->container->get('security.authorization_checker')->isGranted('IS_AUTHENTICATED_FULLY')) {
            $user = $this->container->get('security.token_storage')->getToken()->getUser();
        ...
        return $this->render('MessageBundle:Message:msgDiscussion.html.twig', array(
            'messages' => $messages,
            'count' => $count));
    }
    ...
    public function getMsgFTAction()
    {
        ...
        return $this->render('MessageBundle:Mback:getForTrashFront.html.twig', array(
            'messages' => $messages,
            'count' => $count,
        ));
    }
    ...
    public function getMsgFSAction()
    {
        ...
        return $this->render('MessageBundle:Mback:getForSpamFront.html.twig', array(
            'messages' => $messages,
            'count' => $count,
        ));
    }
    ...
    public function calculMsgStatsAction(){

        $em = $this->getDoctrine()->getManager();
        //Stat 1

        $countAllMsgQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m ");
        $countAllMsg = $countAllMsgQuery->getSingleScalarResult();

        $countMsgNewQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m WHERE (m.id=m.idMessageBase)");
        $countNewMsg = $countMsgNewQuery->getSingleScalarResult();

        $countSameThreadMsg=$countAllMsg - $countNewMsg;

        //Stat 2
        $countMsgInboxQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m WHERE (m.dossier='Inbox')");
        $countMsgInbox = $countMsgInboxQuery->getSingleScalarResult();

        $countMsgSpamQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m WHERE (m.dossier='Spam')");
        $countMsgSpam = $countMsgSpamQuery->getSingleScalarResult();

        $countMsgTrashQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m WHERE (m.dossier='Trash')");
        $countMsgTrash = $countMsgTrashQuery->getSingleScalarResult();

        $countMsgWorkQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m WHERE (m.label='Work')");
        $countMsgWork = $countMsgWorkQuery->getSingleScalarResult();

        $countMsgPrivateQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m WHERE (m.label='Private')");
        $countMsgPrivate = $countMsgPrivateQuery->getSingleScalarResult();

        $countMsgClientQuery = $em->createQuery("SELECT COUNT(m) from MessageBundle:Message m WHERE (m.label='Client')");
        $countMsgClient = $countMsgClientQuery->getSingleScalarResult();


        return $this->render('MessageBundle:Message:MsgStats.html.twig',array(
            'countAllMsg'=>$countAllMsg,
            'countNewMsg'=>$countNewMsg,
            'countSameThreadMsg'=>$countSameThreadMsg,
            'countMsgInbox'=>$countMsgInbox,
            'countMsgSpam'=>$countMsgSpam,
            'countMsgTrash'=>$countMsgTrash,
            'countMsgWork'=>$countMsgWork,
            'countMsgPrivate'=>$countMsgPrivate,
            'countMsgClient'=>$countMsgClient,
        ));
    }
}
